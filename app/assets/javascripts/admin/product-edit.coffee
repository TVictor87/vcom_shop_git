...
@WarehouseTable =
	...
	edit: ->
		...
		for i in [0..count - 4]
			...
			current.title = ''
			isColor = td.dataset.type is 'color'
			...
			if isColor and not select.listenersAdded
				...
				$(select.children[0]).on 'select2:select', (e) ->
	...
	cancel: ->
		...
		for i in [0..count - 4]
			...
			id = td.dataset.id
			value = select.children[0].querySelector("[value='#{id}']").innerHTML
			td.innerHTML = td.dataset.type is 'color' and colorValue(value) or value
		for i in [count - 3..count - 2]
			td = ch[i]
	...
	update: ->
		...
		for i in [0..count - 4]
			td = ch[i]
			option_tds[i] = td
			ids[i] = +td.firstChild.firstElementChild.querySelector(':checked').value
